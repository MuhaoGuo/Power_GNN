...
class VCN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, edge_attr):
        ...
        x = global_mean_pool(x=x, batch = batch)  # [batch_size, hidden_channels]
        # print("x", x.shape)
...
class VmagConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        # if x is a matrix of size (N, F), where N is the total number of nodes in the graph and F is the number of node features, then x represents the node features of the graph.
        # x has shape [N, F_in]
        # edge_index has shape [2, E]
        # edge_attr has shape [E, F_e]
        num_nodes = int(edge_index.max()) + 1
        # print("VmagConv x", x.shape)
        ...
        x_original = x.detach()
        self.x_original = copy.deepcopy(x_original)
    ...
    def propagate(self, edge_index, x, edge_attr, num_nodes):
        ...
        # print("neighbor_terms", neighbor_terms.shape)
        ...
        # print("aggregated_neighbor_terms", aggregated_neighbor_terms.shape)
        ...
        # print("propagate out", out.shape)
        return out
    # todo delete
    # def aggregate(self, inputs, index, dim_size):
    #     print("aggregate inputs", inputs.shape) #([4, 3])  #([E, #features])
    #     print("aggregate index", index.shape)   # torch.Size([4])   , E
    #     print("aggregate dim_size", dim_size)   #3
    #     # print(scatter(inputs, index, -1,  reduce=self.aggr))
    #     return
        # return scatter(inputs, index, -1, dim_size=dim_size, reduce=self.aggr)

    def message(self, edge_index, x, edge_attr):
        # print("message x", x.shape)   #  Size([3, 3])
        # print("edge_attr", edge_attr.shape)   #torch.Size([4, 1])
        # print("edge_index", edge_index.shape) # torch.Size([2, 4])
        ...
        # print("edge_attr.view(1,-1)", edge_attr.view(1,-1).shape)
        # neighbor_terms = edge_attr.view(1,-1).flatten() * x[col,0] * x[col,2]
        # todo 改了   `
        # neighbor_terms = edge_attr.view(1,-1) * x[col,0] * x[col,2]
        x_j = x[edge_index[1]]
        x_i = x[edge_index[0]]
        # print("x_j", x_j.shape)
        # neighbor_terms = self.MLP_for_message(torch.cat([x, edge_attr], dim=1))
        # print("neighbor_terms", neighbor_terms.shape) #(E, out_channels)
